...
<%@ attribute name="onchangeCallback" type="java.lang.Boolean" required="false" description="内容变更时的回调函数"%>
...
<input id="${id}Id" name="${name}" class="${cssClass}" type="hidden" value="${value}"/>
    <input id="${id}Name"   name="${labelName}" ${allowInput?'':'readonly="readonly"'} type="text" value="${labelValue}" data-msg-required="${dataMsgRequired}"
		class="form-control" style="${cssStyle}" <c:if test="${onchangeCallback != '' and onchangeCallback != null }"> onchange="${onchangeCallback}" </c:if> />
